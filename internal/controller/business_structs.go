...
type Business struct {
	...
	BusinessAvailableTime []AvailableTime    `json:"business_available_time"`
	IsAvailable           string             `json:"business_is_available"`
...
type (
	GetBusinessRequest struct {
		LocationLat float32 `json:"location_lat" validate:"required"`
		LocationLon float32 `json:"location_lng" validate:"required"`
	...
	GetBusinessByNameRequest struct {
		...
		Open         int    `json:"open" validate:"required"`
		StartNum     string `json:"start_num" validate:"required"`
